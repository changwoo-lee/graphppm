...
#' 
#' 
#' #' Draw uniform spanning tree compatible to current partition
#' #' 
#' #' g0 must have vid and eid
#' #' E0 must have eid as a rowname
#' #'
#' #' @param z partition 
#' #' @param g0 igraph object, must have vid and eid
#' #' @param E0 m by 2 matrix, must have eid as a rowname
#' #'
#' #' @return
#' #' @export
#' #'
#' #' @examples
#' get_ust_compatible_to_z <- function(z, g0, E0){
#'   
#'   iscrossing = get_iscrossing(z, E0)
#'   
#'   edgelist = list()
#'   # sample ust from quotient graph, tracking eids
#'   # I will use igraph::sample_spanning_tree to keep track of eids...
#'   quotient_edgelist = make_quotient_graph_edgelist(z, E0)
#'   if(nrow(quotient_edgelist) > 0){
#'     quotient_igraph = make_quotient_graph(z, E0)
#'     E(quotient_igraph)$eid = as.integer(rownames(quotient_edgelist))
#'     quotient_tree= igraph::sample_spanning_tree(quotient_igraph)
#'     edgelist[[1]] = quotient_edgelist[as.character(quotient_tree$eid),, drop = F] # can be improved...
#'   }else{
#'     edgelist[[1]] = matrix(0, 0, ncol = 2)
#'   }
#'   
#'   # sample ust componentwise
#'   k = length(unique(z))
#'   for(j in 1:k){
#'     Gj = igraph::induced_subgraph(g0, vids = which(z==j))
#'     nj = vcount(Gj)
#'     if(nj == 1){
#'       edgelist[[j+1]] = matrix(0, 0, ncol = 2)
#'       next;
#'     }
#'     vid = V(Gj)$vid
#'     edges = graphppm:::runcppWilson(nj, as_edgelist(Gj), 1, 1:nj, max(100,nj^3))
#'     edges[,1] = vid[edges[,1]]
#'     edges[,2] = vid[edges[,2]]
#'     edgelist[[j+1]] = edges # first one is quotient
#'   }
#'   tree = do.call(rbind, edgelist)
#'   tree = cbind(tree, 0)
#'   if(nrow(edgelist[[1]]) > 0)  tree[1:nrow(edgelist[[1]]),3] = 1
#'   colnames(tree) = c("v1","v2","iscrossing")
#'   rownames(tree) = NULL
#'   return(tree)
#' }
#' 
