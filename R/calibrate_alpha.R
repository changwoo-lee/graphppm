#' Calibration of alpha based on the number of clusters
#'
...
#'
#' @examples
calibrate_alpha_ncluster <- function(fit, alpha){
...
# fit_tau = graphppm_prior_impsamp(karate,
#                                    logcohesion = logcohesion_tau,
#                                    cohesion_param = cohesion_param,
#                                    nsave = 50000)
# 
# out = calibrate_alpha_ncluster(fit_tau, alpha = seq(0.1, 10, length = 20))
# 
# plotrix::plotCI(seq(0.1, 10, length = 20),
#                 out$ncluster_mean, 
#                 ui = out$ncluster_mean + 2*out$ncluster_mean_sd,
#                 li = pmax(1, out$ncluster_mean - 2*out$ncluster_mean_sd))
# 
# fit_vetau = graphppm_prior_impsamp(karate,
#                                  logcohesion = logcohesion_vetau,
#                                  cohesion_param = cohesion_param,
#                                  nsave = 10000)
# 
# out = calibrate_k_alpha(fit_vetau, alpha = seq(0.1, 10, length = 20))
# 
# plotrix::plotCI(seq(0.1, 10, length = 20),
#                 out$mean, ui = out$mean + 2*out$sd, li = pmax(1, out$mean - 2*out$sd))
# 
